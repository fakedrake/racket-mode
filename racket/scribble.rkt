...
(define ht-documented-exports-promise (delay (make-documented-exports-index)))

(define (documented-export-names)
  (sort (hash-keys (force ht-documented-exports-promise))
        string<?))

;; Convert the set into a list sorted by libs alphabetically.
(define (sort-lib-set s)
  (sort (set->list s)
        string<?
        #:cache-keys? #t
        #:key
...
(define (libs+paths+anchors-exporting-documented sym-as-str)
  (sort-lib-set
   (hash-ref (force ht-documented-exports-promise)
             sym-as-str
             (set))))
